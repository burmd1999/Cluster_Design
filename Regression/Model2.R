...
X <- cbind(rep(1,n),c(rep(-0.5,n/4),rep(-0.2235,n/4),rep(0.2235,n/4),rep(0.5,n/4)))
...
X <- cbind(rep(1,n),c(sort(rep(round(Computexj(m=4,model = 2),3),n/4))))
...
#Huber's Implemented Design
X <- cbind(rep(1,n),c(round(Computexj(m=36,model = 2),3)))
...
#Save results as RDS
model2results <- list(BiasVarTablekC = BiasVarTable.1,BiasVarTablekU = BiasVarTable.2,
...
saveRDS(model2results,file = paste("model",model,"results.rds",sep = ""))
...
BiasVarTable.1 <- cbind(rep(c(NA,"$\\xi_{CDD}^{(3)}$","$\\xi_{H}^{(4)}$","$\\xi_{Cl}$","$\\xi_{U}$"),length(sigma2vector)),BiasVarTable.1)
...
colnames(EffTable.1) <- c("$\\sigma^2$","$k_C$","$p_C$","Eff($\\xi_{Cl}$,$\\xi_{CDD}^{(3)}$)",
...
BiasVarTable.2 <- cbind(rep(c(NA,"$\\xi_{CDD}^{(3)}$","$\\xi_{H}^{(36)}$","$\\xi_{Cl}$","$\\xi_{U}$"),length(sigma2vector)),BiasVarTable.2)
...
colnames(EffTable.2) <- c("$\\sigma^2$","$k_U$","$p_U$","Eff($\\xi_{Cl}$,$\\xi_{CDD}^{(3)}$)",
