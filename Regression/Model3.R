...
results[,3] <- kaccept(X,sigma2vector,n,N = 100,maxBplus = 3,step = 0.02,model = model)
...
#Tables for latex
BiasVarTable.1 <- matrix(NA,nrow = length(sigma2vector)*5,ncol = 7)
...
for (i in 1:length(sigma2vector)){
  ...
  BiasVarTable.1[5*(i-1)+2, 1:7] <- c(resultsCDD.1[i,c(3,4,5,6,7,8,10)])
  BiasVarTable.1[5*(i-1)+3, 1:7] <- c(resultsH4.1[i,c(3,4,5,6,7,8,10)])
  BiasVarTable.1[5*(i-1)+4, 1:7] <- c(resultsCl.1[i,c(3,4,5,6,7,8,10)])
  BiasVarTable.1[5*i, 1:7] <- c(resultsU.1[i,c(3,4,5,6,7,8,10)])
}
EffTable.1 <- matrix(NA,nrow = length(sigma2vector),ncol = 6)
colnames(EffTable.1) <- c("sigma^2","k-C","p-C","CDD/CL","H4/CL","U/CL")
...
EffTable.1[,6] <- resultsU.1[,10]/resultsCl.1[,10]
...
resultsH36.2 <- resultstable(X,n,N,sigma2vector,kvector = results[,5],Cldesign = F,pvector = F,model = model)
...
BiasVarTable.2 <- matrix(NA,nrow = length(sigma2vector)*5,ncol = 7)
colnames(BiasVarTable.2) <- c("BiasB0","BiasB1","BiasB2","VarB0","VarB1","VarB2","det(MSE)")
for (i in 1:length(sigma2vector)){
  BiasVarTable.2[5*(i-1)+1, 1:4] <- c(results[i,c(1,5,6)],N)
  BiasVarTable.2[5*(i-1)+2, 1:7] <- c(resultsCDD.2[i,c(3,4,5,6,7,8,10)])
  BiasVarTable.2[5*(i-1)+3, 1:7] <- c(resultsH36.2[i,c(3,4,5,6,7,8,10)])
  BiasVarTable.2[5*(i-1)+4, 1:7] <- c(resultsCl.2[i,c(3,4,5,6,7,8,10)])
  BiasVarTable.2[5*i, 1:7] <- c(resultsU.2[i,c(3,4,5,6,7,8,10)])
}
#Efficiency Comparison
EffTable.2 <- matrix(NA,nrow = length(sigma2vector),ncol = 6)
colnames(EffTable.2) <- c("sigma^2","k-U","p-U","CDD/CL","H/CL","U/CL")
EffTable.2[,1] <- sigma2vector
EffTable.2[,2] <- results[,5]
EffTable.2[,3] <- results[,6]
EffTable.2[,4] <- resultsCDD.2[,10]/resultsCl.2[,10]
EffTable.2[,5] <- resultsH36.2[,10]/resultsCl.2[,10]
EffTable.2[,6] <- resultsU.2[,10]/resultsCl.2[,10]


#Save results as RDS
model3results <- list(BiasVarTablekC = BiasVarTable.1,BiasVarTablekU = BiasVarTable.2,
                      ...
                      resultsH4kC = resultsH4.1,resultsH36kU = resultsH36.2,
                      resultsUkC = resultsU.1,resultsUkU = resultsU.2)
saveRDS(model3results,file = paste("model",model,"results.rds",sep=""))
...
for (i in 1:length(sigma2vector)){
  BiasVarTable.1[5*(i-1)+2, 1:7] <- customround(c(resultsCDD.1[i,c(3,4,5,6,7,8,10)]))
  BiasVarTable.1[5*(i-1)+3, 1:7] <- customround(c(resultsH4.1[i,c(3,4,5,6,7,8,10)]))
  BiasVarTable.1[5*(i-1)+4, 1:7] <- customround(c(resultsCl.1[i,c(3,4,5,6,7,8,10)]))
  BiasVarTable.1[5*i, 1:7] <- customround(c(resultsU.1[i,c(3,4,5,6,7,8,10)]))
}
BiasVarTable.1 <- cbind(rep(c(NA,"$\\xi_{CDD}^{(2x2)}$","$\\xi_{H}^{(4)}$","$\\xi_{Cl}$","$\\xi_{U}$"),length(sigma2vector)),BiasVarTable.1)
...
for (i in 1:length(sigma2vector)){
  ...
  addtorow$command[i] <- paste("& \\multicolumn{6}{l}{\\underline{$\\sigma^2 = ",BiasVarTable.1[(5*i-4),2],"$,
...
colnames(EffTable.1) <- c("$\\sigma^2$","$k_C$","$p_C$","Eff($\\xi_{Cl}$,$\\xi_{CDD}^{(2x2)}$)",
                          "Eff($\\xi_{Cl}$,$\\xi_{H}^{(4)}$)","Eff($\\xi_{Cl}$,$\\xi_{U}$)")
...
for (i in 1:length(sigma2vector)){
  BiasVarTable.2[5*(i-1)+2, 1:7] <- customround(c(resultsCDD.2[i,c(3,4,5,6,7,8,10)]))
  BiasVarTable.2[5*(i-1)+3, 1:7] <- customround(c(resultsH36.2[i,c(3,4,5,6,7,8,10)]))
  BiasVarTable.2[5*(i-1)+4, 1:7] <- customround(c(resultsCl.2[i,c(3,4,5,6,7,8,10)]))
  BiasVarTable.2[5*i, 1:7] <- customround(c(resultsU.2[i,c(3,4,5,6,7,8,10)]))
}
BiasVarTable.2 <- cbind(rep(c(NA,"$\\xi_{CDD}^{(2x2)}$","$\\xi_{H}^{(36)}$","$\\xi_{Cl}$","$\\xi_{U}$"),length(sigma2vector)),BiasVarTable.2)
colnames(BiasVarTable.2) <- c(" ","$B(\\hat{\\beta_0})$","$B(\\hat{\\beta_1})$","$B(\\hat{\\beta_2})$","$V(\\hat{\\beta_0})$",
                              "$V(\\hat{\\beta_1})$","$V(\\hat{\\beta_2})$","det(MSE)")
addtorow <- list()
for (i in 1:length(sigma2vector)){
  addtorow$pos[i] <- list(4*(i-1))
  addtorow$command[i] <- paste("& \\multicolumn{6}{l}{\\underline{$\\sigma^2 = ",BiasVarTable.2[(5*i-4),2],"$,
                             $k_U = ",BiasVarTable.2[(5*i-4),3],"$,
                             $p_U = ",BiasVarTable.2[(5*i-4),4],"$,
                             $n = ",n,"$}} & \\\\",sep = "")
}
BiasVarTable.2 <- BiasVarTable.2[-seq(1,length(sigma2vector)*5,by=5),]
table1 <- xtable(BiasVarTable.2,caption = "Table of Bias, Variance and D-optimality criteria.",align=c("|c","|c","|c","|c","|c","|c","|c","|c","|c|"),
                 digits = c(0,0,5,5,5,5,5,5,5))
...
colnames(EffTable.2) <- c("$\\sigma^2$","$k_U$","$p_U$","Eff($\\xi_{Cl}$,$\\xi_{CDD}^{(2x2)}$)",
                          "Eff($\\xi_{Cl}$,$\\xi_{H}^{(36)}$)","Eff($\\xi_{Cl}$,$\\xi_{U}$)")
...
X <- cbind(rep(1,n),
           ...
           rep(c(0.5,0.5-0.25*sqrt(p),0.5-0.5*sqrt(p),-0.5+0.5*sqrt(p),
                 -0.5+0.25*sqrt(p),-0.5),6)  )
plot(X[,2],X[,3], main = "Bivariate Uniform Design, n=36",ylab = "x2",xlab="x1",
     xaxp=c(-0.5,0.5,2),yaxp=c(-0.5,0.5,2),pch=19)
abline(h=0,lty=3)
abline(v=0,lty=3)
#Clustered
p <- 0.05
X <- cbind(rep(1,n),
           ...
           rep(c(0.5,0.5-0.25*sqrt(p),0.5-0.5*sqrt(p),-0.5+0.5*sqrt(p),
                 -0.5+0.25*sqrt(p),-0.5),6)  )
plot(X[,2],X[,3], main = "Bivariate Clustered Design, p=0.05, n=36",ylab = "x2",xlab="x1",
     xaxp=c(-0.5,0.5,2),yaxp=c(-0.5,0.5,2),pch=19)
abline(h=0,lty=3)
abline(v=0,lty=3)
