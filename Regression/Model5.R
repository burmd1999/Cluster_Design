#Model 5 - Quadratic Model with complex contamination function
cat('\f')
rm(list=ls())
#Retrieve functions
...
results[,3] <- kL2(n,sigma2vector,maxk = 8,model = 5)
results[,4] <- Optp(n,N=5000,sigma2vector,kvector = results[,3],model = 5,ktype="")
...
#Save results as RDS
model5results <- list(BiasVarTable = BiasVarTable.1,
                      EffTable = EffTable.1,
                      results = results,
                      ...
                      resultsU = resultsU.1)
saveRDS(model5results,file = paste("model",model,"results.rds",sep = ""))
...
BiasVarTable.1 <- cbind(rep(c(NA,"$\\xi_{CDD}^{(3)}$","$\\xi_{H}^{(4)}$","$\\xi_{H}^{(36)}$","$\\xi_{Cl}$","$\\xi_{U}$"),length(sigma2vector)),BiasVarTable.1)
...
colnames(EffTable.1) <- c("$\\sigma^2$","$k$","$p$","Eff($\\xi_{Cl}$,$\\xi_{CDD}^{(3)}$)",
